...
public class World {
	...
	public World() {
		...
		world.setContactListener(new org.jbox2d.callbacks.ContactListener() {
			...
			public void beginContact(org.jbox2d.dynamics.contacts.Contact contact) {
				...
				container.first = new Contact((Part) contact.m_fixtureA.m_userData,
						(Part) contact.m_fixtureB.m_userData);
				container.second = new Contact((Part) contact.m_fixtureB.m_userData,
						(Part) contact.m_fixtureA.m_userData);
	...
	public List<Impact> trace(Vector start, Vector end) {
		...
		RayCastCallback callback = new RayCastCallback() {
			...
			public float reportFixture(Fixture fixture, Vec2 point, Vec2 normal, float fraction) {
				Impact impact = new Impact((Part) fixture.m_userData, new Vector(point.x, point.y),
						new Vector(normal.x, normal.y), fraction);
				impacts.add(impact);
				return -1.0f;
			}
		};
	 ...
	 * 
	 * @param deltaTime
	 *            time elapsed since last update, non-negative
	 */
