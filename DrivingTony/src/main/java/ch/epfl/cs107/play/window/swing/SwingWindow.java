...
public class SwingWindow extends Node implements Window {
	...
	public SwingWindow(String title, FileSystem fileSystem) {
		...
		canvas.setBackground(new Color(190,255,255));
	...
	public void update() {
		...
		Transform projection = new Transform(0.5f * width / halfX, 0.0f, 0.5f * width, 0.0f, -0.5f * height / halfY,
				0.5f * height);
	...
	public void drawText(String text, float fontSize, Transform transform, Color fillColor, Color outlineColor,
		...
		if (transform == null)
			throw new NullPointerException();
		if (text == null || fontSize <= 0.0f || alpha <= 0.0f
				|| (fillColor == null && (outlineColor == null || thickness <= 0.0f)))
			return;
		items.add(new TextItem(text, fontSize, transform, fillColor, outlineColor, thickness, bold, italics, anchor,
				depth, alpha));
	}
