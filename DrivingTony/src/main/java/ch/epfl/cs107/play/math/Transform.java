...
public final class Transform implements Serializable {
	...
	public Vector onPoint(float x, float y) {
		return new Vector(x * m00 + y * m01 + m02, x * m10 + y * m11 + m12);
	}
	...
	public Vector onVector(float x, float y) {
		return new Vector(x * m00 + y * m01, x * m10 + y * m11);
	}
	...
	public Transform transformed(Transform t) {
		return new Transform(t.m00 * m00 + t.m01 * m10, t.m00 * m01 + t.m01 * m11, t.m00 * m02 + t.m01 * m12 + t.m02,
	...
	public Transform translated(float dx, float dy) {
		return new Transform(m00, m01, m02 + dx, m10, m11, m12 + dy);
	}
	...
	public Transform scaled(float sx, float sy) {
		return new Transform(m00 * sx, m01 * sx, m02 * sx, m10 * sy, m11 * sy, m12 * sy);
	}
	...
	public Transform rotated(float a) {
		...
		return new Transform(c * m00 - s * m10, c * m01 - s * m11, c * m02 - s * m12, s * m00 + c * m10,
				s * m01 + c * m11, s * m02 + c * m12);
	}
	 ...
	 *            angle, in radians
	 * @param center
	 *            rotation axis, not null
	 * @return extended transform, not null
	 */
	public Transform rotated(float a, Vector center) {
		return translated(-center.x, -center.y).rotated(a).translated(center);
	}
	...
	public Transform inverted() {
		...
		float c = -m10 * det;
		float d = m00 * det;
		return new Transform(a, b, -(a * m02 + b * m12), c, d, -(c * m02 + d * m12));
	}

	@Override
	public int hashCode() {
		return Float.hashCode(m00) ^ Float.hashCode(m01) ^ Float.hashCode(m02) ^ Float.hashCode(m10)
				^ Float.hashCode(m11) ^ Float.hashCode(m12);
	}
	...
	public boolean equals(Object object) {
		...
		return m00 == other.m00 && m01 == other.m01 && m02 == other.m02 && m10 == other.m10 && m11 == other.m11
				&& m12 == other.m12;
	}
